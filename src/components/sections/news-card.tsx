...
const NewsCard: React.FC<NewsCardProps> = ({ newsItem, className }) => {
  ...
  return (
    <Link href={`/news/${slug}`} className={cn("group block bg-card rounded-lg overflow-hidden shadow-lg hover:shadow-primary/20 transition-all duration-300 transform hover:-translate-y-1", className)}>
      <div className="relative aspect-video w-full bg-muted">
        <Image
          src={imageUrl || '/assets/images/events/cs_championship.webp'}
          alt={title}
          fill
